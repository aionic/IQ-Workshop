...
The architecture is: **Foundry Prompt Agent (gpt-4.1-mini) → MCP Server (Streamable HTTP) → FastAPI Tool Service (Container Apps) → Azure SQL**.
...
- **Foundry Prompt Agent** — gpt-4.1-mini in Foundry Agent Service. Registered with `McpTool` (azure.ai.agents.models) pointing at the MCP server. Does NOT access the database directly.
- **MCP Server** — FastMCP co-hosted on the FastAPI app at `/mcp` (Streamable HTTP transport). Primary integration path. Foundry Agent Service connects directly — no client-side tool loop needed.
- **Client Tool Loop (Legacy)** — `chat_agent.py` (using `AgentsClient`) supports `--legacy` mode: intercepts `requires_action`, calls REST endpoints, and submits outputs back. Kept for backward compat.
- **FastAPI Tool Service** — Python FastAPI app self-hosted on Azure Container Apps (port 8000). REST endpoints (`/tools/*`) are deprecated; MCP at `/mcp` is primary. This is the ONLY component that writes to the database.
...
- **Phase 5:** MCP Integration (FastMCP co-hosted at `/mcp`, McpTool agent registration, Streamable HTTP)
- **Phase 6:** Feature Enhancements — planned (agent memory, knowledge grounding, Foundry portal evals, Python bump)
...
| New MCP tool | `services/api-tools/app/mcp_server.py`, `services/api-tools/app/db.py` (if new query), `services/api-tools/tests/test_mcp_server.py`, `foundry/agent.yaml` |
...
- Upload evals to Foundry: `uv run evals/upload_to_foundry.py --resource-group rg-iq-lab-dev`
