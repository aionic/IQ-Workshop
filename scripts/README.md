...
This creates a gpt-4.1-mini prompt agent with **Responses API compatible function tools**.
Tool definitions are auto-generated by `FunctionTool` from Python callables + docstrings
(Agent Framework v2 SDK). Tool calls are handled client-side by `chat_agent.py`.

**Option A — Agent Framework v2 SDK (recommended, automated via `uv run`):**
```bash
...
| Script | Purpose | Key Flags |
|--------|---------|-----------|
| `create_agent.py` | Agent Framework v2 SDK agent creation (PEP 723 deps) | `--resource-group` or env vars |
